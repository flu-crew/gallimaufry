...
inp_20b = open("offlu_2020b_h1_var.fna")
inp_21a = open("offlu_2021a_h1_var.fna")
inp_21b = open("offlu_2021b_h1_var.fna")
inp_22a = open("offlu_2022a_h1_var.fna")
inp_22b = open("offlu_2022b_h1_var.fna")
inp_23a = open("offlu_2023a_h1_var.fna")
out = open("offlu_h1_var.fna", "w")
...
def ProcessFasta(fd, report):
    ...
    for defline, seqs in inpDict.items():
        if len(seqs) > 1:
            ...
            strain = defLst[-6]
            sub = defLst[-5].replace("N1v","N1").replace("N2v","N2").replace("Nx","")
            ...
            if clade in ["","1A.3.3.3"]:
                newdef = "variant|%s|%s|Human|%s|%s|%s" % (strain, sub, cntry, clade, date)
