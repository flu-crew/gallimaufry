setwd("/Users/david.hufnagel/Documents/2_Research/1_2020/5_N1SelectionReborn/7_HApairing/1_2yrDataOverTime")

#import data
inp = read.table("pairData_2yr_Rinp.txt", sep="\t")

#split up data by NA clade and make a plot for each NA clade
inpSplit = split(inp, inp$V1)
for (naData in inpSplit){
  plotName = sprintf("pairingOT_%s.pdf", naData$V1[1])
  plot.new()
  pdf(plotName)
  ...
  for (haData in naDataSplit){
    if (cnt == 0){
      ...
      plot(X[!is.na(Y)], Y[!is.na(Y)], type="l", xlim=c(0,27), ylim=c(0, 1), lwd=2, bty="n", xaxt="n", yaxt="n", cex.lab=1.2, xlab="time (months since Jan 2018)", ylab="% HA pairing")
      axis(side = 1, lwd = 2, font=2); axis(side = 2, lwd = 2, font=2)
      
      #colors = c(colors, "black"); has = c(has, haData$V2[cnt+1]); ltys=
    }
    else
      X = haData$V5; Y = haData$V6
  ...
  #legend("right", legend=has[2:length(has)], pch=15, col=colors[2:length(colors)], lwd=1, lty=])
