...
Created by David E. Hufnagel on Apr  4, 2022
Updated by David E. Hufnagel on Apr 13, 2022 to use stateIDsPoiss.txt as a 
   starting point for clusterIDdict. This results in a shared collection of
   SGs for both the clade and clade pair plots
"""
import sys, copy
...
allDataDict = {}
IngestInput(inp_n1c1x_LAIV, "N1.C.1.X-1A.2-3-like", allDataDict)
...
# instantiate clusterIDdict using the SG output file for NA clades
clusterIDdict = {} #a dict of key: state cluster  val: cluster id
topIDnum = 1     #the number used to generate state cluster IDs
for line in inp_sg:
    lineLst = line.strip().split("\t")
    #output original
    out_stateID.write(line)
    
    #update clusterIDdict
    clusterIDdict[lineLst[1]] = lineLst[0]
    
    #update topIDnum
    sgNum = int(lineLst[0].strip("SG"))
    if sgNum > topIDnum:
        topIDnum = sgNum
topIDnum += 1
...
inp_n1c32_1a333.close(); inp_n1p_1a332.close(); inp_sg.close()
out_all.close(); out_Up_RR2.close(); out_stateID.close()
