...
Modified on May 18, 2021 to work with merged data
"""
import sys

aaDiffs = open("gammaProjMergedData_all_noACC_pdmGamma_MN45fix_trim_dists.csv")
hiDiffs = open("mergedData_distances_all.csv")
outAll = open("gammaMerged_compareAAdiffHIdiff_1A.3.3.X.tab", "w")
outPdm = open("gammaMerged_compareAAdiffHIdiff_1A.3.3.2.tab", "w")
outGamma = open("gammaMerged_compareAAdiffHIdiff_1A.3.3.3.tab", "w")
...
strains = []; cladeDict = {}
for defline in aaDtList:
    if defline != "":
        strain = ChangeStrainNames(defline.split("|")[0].upper())
        strains.append(strain)
        clade = defline.split("|")[4]
        cladeDict[strain] = clade


##Collect aaDif data in a dict of key: strainA_strainB  val: aaDiff
aaDiffDict = {}
for line in aaDiffs:
    ...
    for val in lineLst[1:]:
        ...
        key = "%s__%s" % (strainA.upper(), strainB.upper())
...
for strain in strains:
    strain2 = ChangeStrainNames(strain)
    strains2.append(strain2)
...
for line in hiDiffs:
    ...
    if not "SR" in lineLst[0] + lineLst[2]:
        ...
        key1 = "%s__%s" % (strainA, strainB)
        key2 = "%s__%s" % (strainB, strainA)
        if key1 in aaDiffDict:
            ...
            cladeA = cladeDict[strainA]
            cladeB = cladeDict[strainB]
            
            newline = "%s\t%s\t%s\t%s\t%s\t%s\n" % \
                (lineLst[0], lineLst[1], lineLst[2], lineLst[3], lineLst[4], aaDiffDict[key1])
            if cladeA  == cladeB == "1A.3.3.2":
                outPdm.write(newline)
            elif cladeA == cladeB == "1A.3.3.3":
                outGamma.write(newline)
            outAll.write(newline)
