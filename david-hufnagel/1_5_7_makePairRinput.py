...
inp = open(sys.argv[1])      #pairData_2yr.txt
out = open(sys.argv[2], "w") #pairData_2yr_Rinp.txt
...
#Go thorugh input file and build a dict of key: NA clade val: [Year1Lst,
...
#  in the format:  naClade   haClade   year   month   monthNum   %pair
for naClade, yearsLst in inpDict.items():
    yCnt = 0
    for yearLst in yearsLst:
        ...
        for monthLst in yearLst:  
            ...
            for pair in pairData:
                haClade = pair[0]; perHA = pair[1]
                if monthNum < 28:  #cut off R input where there is no data
                    newline = "%s\t%s\t%s\t%s\t%s\t%s\n" % (naClade, haClade, year, mCnt, monthNum, perHA)
                    out.write(newline)
