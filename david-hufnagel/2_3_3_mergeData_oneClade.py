...
# -*- coding: utf-8 -*-
"""
This script is designed to combine aadiff information and HI information from
a particular clade into one file.
...
out = open(sys.argv[3], "w") #gamma_compareAAdiffHIdiff_1A333.tab
goodClade = sys.argv[4]      #1A.3.3.3
...
for defline in aaDtList:
    if defline != "":
        strain = defline.split("|")[0].upper()
        strains.append(strain)
        
        clade = defline.split("|")[4]
        cladeDict[ChangeStrainNames(strain)] = clade
...
title = "ID1\tStrain1\tID2\tStrain2\tHIdist\taaDist(%)\tclade\n"
out.write(title)
...
for line in hiDiffs:
    ...
    if not "SR" in lineLst[0] + lineLst[2]:
        ...
        if key1 in aaDiffDict:
            ...
            if cladeA == cladeB == goodClade:
              newline = "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % \
                  (lineLst[0], lineLst[1], lineLst[2], lineLst[3], lineLst[4], aaDiffDict[key1], cladeA)
              out.write(newline)
            ...
            if cladeA == cladeB == goodClade:
                newline = "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % \
                    (lineLst[0], lineLst[1], lineLst[2], lineLst[3], lineLst[4], aaDiffDict[key2], cladeA)
                out.write(newline)
