...
satData = read.table("statsPoissResults_clades_up_rr2.txt", header = TRUE, 
...
plotFrame = data.frame(Position = satData$clusterID, 
                       Name = satData$clade, start = satData$startDate, 
                       end = satData$endDate, color = satData$Rcolor, 
                       rr = satData$RR, cas = satData$numCases)

#Plot data
plot.new()
pdf("satscanPoissonClades.pdf", height=5, width=7)
...
plot.new()
pdf("satscanPoissonClades_colorLegend.pdf", height=5, width=3)
...
legend(list(x=170, y=200), c("4-8","8-15","15-21","21-77"), title="RR:",
...
plot.new()
pdf("satscanPoissonClades_clusterLegend.pdf", height=7, width=6)
#make a fake plot using the same data and add a legend to it (the legend for state cluster names)
plot(plotFrame$rr ~ plotFrame$cas, col=plotFrame$color)
legend(list(x=148, y=77), c(" SG01 = AL,AR,IL,LA,MO,MS,OK,TN","SG02 = NE,SD",
                           "  SG03 = AL,AR,DC,DE,GA,IL,IN,KY,MD,MI,MO,", "               MS,NC,OH,PA,SC,TN,VA,WI,WV",
                           "SG04 = NC,VA", "SG05 = IA,IL,WA", "  SG06 = AK,AZ,CA,CO,HI,ID,KS,MN,MT,ND,",
                           "               NE,NM,NV,OK,OR,SD,TX,UT,WA,WY",
                           " SG07 = IL,IN,KY,NC,OH,SC,TN,VA,WV","SG08 = MN,ND,SD",
                           "  SG09 = AL,AR,AZ,CO,KS,LA,MO,MS,NE,NM,OK,TX", "SG10 = IN,KY,OH", 
                           "  SG11 = AR,AZ,CA,CO,ID,KS,MO,MT,NC,ND,", "               NE,NM,NV,OK,OR,TX,UT,WA,WY", 
                           "SG12 = IL,MI,WI", "  SG13 = CT,DC,DE,IL,IN,KY,MA,MD,ME,MI,NC,", 
                           "               NH,NJ,NY,OH,PA,RI,SC,VA,VT,WI,WV", " SG14 = KS,MN,ND,NE,SD", 
                           " SG15 = AZ,CO,NM,OK,TX,UT", " SG16 = AR,LA,MO,MS,TN"), 
       pch="", box.lwd=1.2, lty=0, cex=0.7, pt.cex=0.0, y.intersp=1.1, adj=0.04)
dev.off()
