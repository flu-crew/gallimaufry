...
data_all = read.table("allN1s_v4_stateData_wOther.tab")
...
plot.new()
ragg::agg_tiff("stateBarPlot.tiff", height=7, width=7, units="in", res=350)
ggplot(toPlot, aes(fill=regions, y=freqs, x=groups)) +
    geom_bar(position="fill", stat="identity") + 
    theme(axis.text.x = element_text(angle = 90)) +
    labs(x = "NA clades", y = "US State Frequency") +
    ggtitle("The Distribution of US States Within N1 NA Clades") + 
    theme(plot.title = element_text(hjust = 0.5)) +
    guides(fill=guide_legend(title="US States"))
dev.off()
