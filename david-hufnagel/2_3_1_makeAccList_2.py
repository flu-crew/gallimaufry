...
Updated on May 13, 2021 to use a merged data file that includes 616C data
   as well as other data
"""
import sys

accFd = open("AC_viruses_swine_sera_standardization-updated_20201118_normal.txt")
strainFd = open("MergedDataFiles/mergedData_distances_all.csv")
out = open("MergedDataFiles/mergedData_distances_all_accs.txt", "w")
...
#cleanup dict so that there's only one accession per strain name
accDict2 = {}
    ...
    if not name in accDict.keys():
        if not name.replace("_", " ") in accDict.keys():            
            ...
            if name == "A/CALIFORNIA/4/2009":
                acc = "MN371616"
            elif name == "A/SWINE/SASKATCHEWAN/SD0102/2015":
                acc = "MF768539"
            elif name == "A/SWINE/SASKATCHEWAN/SD0200/2016":
                acc = "MF768547"
        else:
